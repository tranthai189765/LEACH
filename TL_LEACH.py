...
import random
def select_cluster_heads_2(network, P):
    maximum_round = int(1/P)
    last_cluster_heads_2 = network.cluster_heads_2_buffer.take(maximum_round)
    print("last_cluster_heads_2 : ", len(last_cluster_heads_2))
    candidate_nodes = [node for node in network.available_nodes 
                       if not node.is_sink and node.id not in last_cluster_heads_2]
...
def run(network, P, K1, K2, display=False):
    select_cluster_heads_2(network, P)
